...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/13 15:04:23 by saxiao            #+#    #+#             */
/*   Updated: 2020/01/13 17:20:31 by saxiao           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_fd(char *name)
{
	...
	if (S_ISDIR(buf.st_mode))
		put_error_with_return(name, ": Is a direcrory.\n");
	return (fd);
}

char		*get_nlist(char *p, unsigned int i, int x, struct symtab_command *s)
{
	unsigned long size;

	size = x ? sizeof(struct nlist_64) : sizeof(struct nlist);
	return (p + s->symoff + i * size);
}
